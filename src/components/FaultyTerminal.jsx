// src/components/FaultyTerminal.js
...
float digit(vec2 p) {
  ...
  if(uUseMouse > 0.5) {
    ...
    intensity += mi;
    float ripple = sin(d * 20.0 - iTime * 5.0) * 0.1 * mi;
    intensity += ripple;
  }
  ...
  if(uUsePageLoadAnimation > 0.5) {
    ...
    float prog = clamp((uPageLoadProgress - cellDelay) / 0.2, 0.0, 1.0);
    intensity *= smoothstep(0.0,1.0,prog);
  }
...
vec3 getColor(vec2 p) {
  ...
  vec3 baseColor = vec3(0.9)*m + sum*0.1*vec3(1.0)*bar;
  return baseColor;
}
...
export default function FaultyTerminal({
  ...
  useEffect(()=>{
    ...
    const update = t=>{
      ...
      if(pageLoadAnimation && loadStartRef.current>0) {
        const dur=2000;
        const prog = Math.min((t-loadStartRef.current)/dur,1);
        program.uniforms.uPageLoadProgress.value = prog;
      }
